...
def parse_db_row(model: type, record: list):
    """
    Using a record from the table corresponding to the class_type,
    checks if an instance exists in .all and, if so, overwrites the
    existing item's attributes. If no matching instance in .all, matches
    the the database record, then instantiates new instance from the
    record's data and adds it to .all.

    Returns the item instance.
    """
    if item := model.all.get(record[0]):
        attrib_names = list(vars(model).keys())[1:]
        attrib_values = record[1:]
        for attrib, value in zip(attrib_names, attrib_values):
            setattr(item, attrib, value)

    else:
        field_list = list(record[1:])
        item = model(*field_list)
        item.id = record[0]
        model.all[item.id] = item
