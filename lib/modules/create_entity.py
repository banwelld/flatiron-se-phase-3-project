import sys
from pathlib import Path
from typing import Union

sys.path.append(str(Path(__file__).resolve().parent.parent))

from models.participant import Participant
from models.team import Team
from modules.get_attr_value import get_attr_value
from util.nav_sentinels import USER_BACK


def collect_user_responses(model_type: str, attr_config: dict) -> dict:
    collection = {}
    for attr_name, attr_val in attr_config.items():
        collection[attr_name] = get_attr_value(
            model_type, attr_val, attr_val.get("display_text")
        )
    return collection


def collect_instantiation_data(
    model: Union[Participant, Team], model_type: str
) -> dict:
    attr_config = generate_attr_config(model)
    return collect_user_responses(model_type, attr_config)


def generate_attr_config(model: Union[Participant, Team]) -> dict:
    """
    Generates a dictionary of attributes required for the instantiation of a model.
    """
    required_attrs = {
        attr_name: attr_val
        for attr_name, attr_val in model.CONFIG.items()
        if attr_val["req_for_init"]
    }
    return required_attrs


def main(model: Union[Participant, Team]) -> Union[Participant, Team]:
    """
    Collects data necessary for instantiating a participant or team and instantiates the entity. If user hits
    CRTL + C returns the escape sentinel to cancel the operation immediately.
    """
    try:
        model_type = model.__name__.lower()
        attr_values = collect_instantiation_data(model, model_type)
        return model.create(**attr_values)
    except KeyboardInterrupt:
        return USER_BACK
